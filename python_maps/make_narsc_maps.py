...
F.AX7AA1990 * 1.0 / (F.AX7AA1990 + F.AX7AB1990) AS povrate90,
F.AX7AA2000 * 1.0 / (F.AX7AA2000 + F.AX7AB2000) AS povrate00,
F.AX7AA125 * 1.0 / (F.AX7AA125 + F.AX7AB125) AS povrate10,	
...
for x in ['90', '00', '10']:
	...
	df.loc[df['povrate{}'.format(x)]<.10, 'povrate_breaks_{}'.format(x)] = 1
	df.loc[(df['povrate{}'.format(x)]>=.10) & (df['povrate{}'.format(x)]<.20), 'povrate_breaks_{}'.format(x)] = 2
	df.loc[(df['povrate{}'.format(x)]>=.20) & (df['povrate{}'.format(x)]<.30), 'povrate_breaks_{}'.format(x)] = 3
	df.loc[(df['povrate{}'.format(x)]>=.30) & (df['povrate{}'.format(x)]<.40), 'povrate_breaks_{}'.format(x)] = 4
	df.loc[(df['povrate{}'.format(x)]>=.40), 'povrate_breaks_{}'.format(x)] = 5
	# print x
...
mylabels3 = ['0.00 - 0.10', '0.10 - 0.20', '0.20 - 0.30', '0.30 - 0.40', '0.40 - 0.63']

print "+" * 100
print df['povrate90'].max()
print df['povrate00'].max()
print df['povrate10'].max()
print "+" * 100

for v in ['diversity_4grp', 'Sus', 'gini', 'povrate']:
# for v in ['Sus']:
	title = 'diversity'
	mylabels = mylabels1
	if v == 'Sus':
...
for v in ['specialization', 'gini', 'diversity', 'povrate']:
# for v in ['specialization']:
	...
	for p in periods:
		...
		df['povrate_diff_{}{}'.format(start,end)] = df['povrate{}'.format(end)] - df['povrate{}'.format(start)]
