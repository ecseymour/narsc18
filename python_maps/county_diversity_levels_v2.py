...
for v in ['specialization', 'gini', 'diversity', 'pwhite', 'povrate']:
# for v in ['pwhite']:
	...
	for p in periods:
		...
		df['pwhite_diff_{}{}'.format(start,end)] = df['pwhite_{}'.format(end)] - df['pwhite_{}'.format(start)]
		df['povrate_diff_{}{}'.format(start,end)] = df['povrate_{}'.format(end)] - df['povrate_{}'.format(start)]
		##############################################################################################################################
		if v in ['diversity', 'specialization', 'povrate']:
		...
		ax=plot_dataframe_custom(df, column='{}_breaks_{}{}'.format(v,start,end), linewidth=.1, edgecolor='white', 
