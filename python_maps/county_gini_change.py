...
qry = '''
SELECT A.gisjoin, B.gini_10, B.gini_00, Hex(ST_AsBinary(C.geometry)) AS geom
FROM us_county_2010 AS A JOIN county_gini AS B 
	ON A.gisjoin = B.gisjoin
JOIN gz_2010_us_050_00_20m AS C 
	ON A.statefp10 = C.state AND A.countyfp10 = C.county
WHERE A.STATEFP10 NOT IN ('02', '15', '72')
AND A.GISJOIN <> 'G0800140'
;
'''

df = gpd.GeoDataFrame.from_postgis(qry, con, geom_col='geom')
df['gini_diff_0010'] = df['gini_10'] - df['gini_00']
print df['gini_diff_0010'].describe()

#######################################################################
ax=df.plot(column='gini_10', linewidth=.1, edgecolor='white', legend=True, categorical=False, scheme='Fisher_Jenks')
...
plt.title("Gini 2012")
outFile = "/home/eric/Documents/franklin/narsc2018/figures/gini_map.png"
plt.savefig(outFile)
plt.close()
#######################################################################
...
# df['chgcat'] = None
df.loc[df['gini_diff_0010'] < 0, 'chgcat'] = 'decrease' 
df.loc[df['gini_diff_0010'] > 0, 'chgcat'] = 'increase' 
df.loc[df['gini_diff_0010'] > df['gini_diff_0010'].std() * 2, 'chgcat'] = 'maj. increase' 
print df.groupby('chgcat').size()

print df.loc[df['gini_diff_0010'].isnull()]
...
leg = ax.get_legend()
leg.set_bbox_to_anchor((0., 0.1, 0.2, 0.2))
plt.title("Difference in Gini 2000 - 2012")
outFile = "/home/eric/Documents/franklin/narsc2018/figures/gini_diffmap.png"
plt.savefig(outFile)
plt.close()