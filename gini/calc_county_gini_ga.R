...
for(i in 1:length(files)){
  ...
  outfile <- glue("/home/eric/Documents/franklin/narsc2018/generated_data/binned_income/county_gini_{survey}_GA.csv") # add to end of output file
  print(outfile)
  ...
  results <- fitFunc(ID = ID, hb = hb, bin_min = bmin, bin_max = bmax, obs_mean = omu, ID_name = 'FIPS', 
    distribution = "GA", distName = "GA", qFunc = qGA)
